...
fields_h.psi_dot = zeros(params.griddim);
fields_h.psi_dot_F = complex(zeros(params.griddim));
...
fields_d.psi_dot = gpuArray(fields_h.psi_dot);wait(gpu);
fields_d.psi_dot_F = gpuArray(fields_h.psi_dot_F);wait(gpu);
...
for step=1:params.maxStep
    ...
    % if step > params.equilTime && step < params.stopStrain
    %     dz = dz * (1 + params.strain_rate*params.dt);
    %     dy = dy/sqrt(1 + params.strain_rate*params.dt);
    %     dx = dx/sqrt(1 + params.strain_rate*params.dt);
    %     [ operators_h, operators_d ] = prepareOperators( params, gpu, dx, dy, dz );
    % end
    ...
    if mod(step,params.outStep)==0
        ...
        if params.fullout_hdf5
            ...
            %read line below more carefully
        ...
        if params.dispOutput
            if params.is3D
                ...
                figure(1);
                clf;
                imagesc(fields_h.psi(:,:,32));colorbar
    ...
    if mod(step,params.printStep)==0
        ...
        disp(  ['### Grid sizes (dx,dy,dz) = (' num2str(dx) ',' num2str(dy) ',' num2str(dz) ') ###'] );
        disp(  ['### Cell volume (dx,dy,dz) = (' num2str(dx*dy*dz) ') ###'] );
        tic
    end
